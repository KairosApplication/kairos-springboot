...
class CategoryControllerIntegrationTest {
    ...
    void shouldRunCrudAndUpdateSameRecord() throws Exception {
        mockMvc.perform(post("/api/v1/categories/registration").with(user("tester").roles("MANAGER")).with(csrf())
        ...
        mockMvc.perform(patch("/api/v1/categories/update/{id}", id).with(user("tester").roles("MANAGER")).with(csrf())
        ...
        mockMvc.perform(get("/api/v1/categories/find/{id}", id).with(user("tester").roles("MANAGER")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.category").value("Alimentos"));
        mockMvc.perform(get("/api/v1/categories/list").with(user("tester").roles("MANAGER")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].category").value("Alimentos"));
        mockMvc.perform(delete("/api/v1/categories/delete/{id}", id).with(user("tester").roles("MANAGER")).with(csrf()))
    ...
    void shouldReturnNotFoundAndConflict() throws Exception {
        mockMvc.perform(get("/api/v1/categories/find/{id}", -1L).with(user("tester").roles("MANAGER")))
    ...
    void shouldRejectUpdateWithoutCategoryName(String body) throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/categories/update/{id}", category.getId()).with(user("tester").roles("MANAGER")).with(csrf())
                .contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.validationErrors.category").value("Categoria é obrigatória"));
    ...
    void shouldReturnNotFoundWhenUpdatingOrDeletingMissingCategory() throws Exception {
        mockMvc.perform(patch("/api/v1/categories/update/{id}", -1L).with(user("tester").roles("MANAGER")).with(csrf())
                .contentType(MediaType.APPLICATION_JSON).content("{\"category\":\"Bebidas\"}"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value("Category not found"));

        mockMvc.perform(delete("/api/v1/categories/delete/{id}", -1L).with(user("tester").roles("MANAGER")).with(csrf()))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value("Category not found"));
    }
}
